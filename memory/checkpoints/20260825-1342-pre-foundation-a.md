...
- Q-002: 로그인 후 실행 대상 Google Cloud 프로젝트와 billing 연결을 선택·확인해야 한다. 예산 한도는 D-012에 따라 불필요하다.
...
Foundation A에서 `gcloud`와 Terraform 설치 경로를 구현·확인한다. Cloud apply 전에는 로그인 계정의 GCP project·billing을 탐색해 allowlist gate를 구성한다.
