# Checkpoint — GitHub private 저장소 생성 — 2026-08-25 13:10

## The story so far

한국어 Google Cloud Lab 01–15 설계와 한국어 root commit이 로컬 main에 있다. 사용자 승인에 따라 `grapefruit0205/gcp-lab-harness` private GitHub 저장소를 빈 상태로 생성하고 로컬 `origin`을 repo 전용 SSH alias로 연결했다. repo 전용 deploy key는 로컬에 생성했고 GitHub Add key 양식까지 준비했지만, write access 권한 부여는 action-time 사용자 확인 전이라 제출하지 않았다. 실제 Cloud adapter는 아직 없다.
...
사용자 승인 후 GitHub deploy key의 write access를 제출하고 로컬 변경을 커밋한 다음 main push와 `pull --ff-only`를 검증한다.
...
- GitHub deploy key 추가는 repository write 권한 변경이므로 양식을 제출하기 직전에 사용자 확인이 필요하다.
