...
| Q-014 | D-036의 보존·진단·수정·재apply 원칙을 실제 자동화의 모든 실행/실패 경로에 어떻게 이관할까? | 2026-08-26 | partial — Phase09 전용 recovery 외에Phase10–15 safe adapter의54회귀·TF mock9/gate와실제apply/검증이통과했다. P10/11/13/14 실제실패에서state/리소스/로그보존·동일run재apply성공을관측했다([실행근거](PRODUCT-TRUTH.md)). Phase01–08/shared adapter의옛자동삭제경로는미이관이므로실행금지유지. Phase08/09승인소스는보존 |
...
| Q-021 | Phase10–15 새 보존형 구현의 실제 Cloud E2E와 원문 대비 남은 차이는 무엇인가? | 2026-08-26 | resolved-current-scope — Phase10–15 전부apply/기계검증통과;13 f5c0dead…자동확장/축소,14 9af07dc…VIP60/두backend 포함.54회귀/TF mock9/전체offline PASS. 전체golden/UI/메일/IPv6실제HTTP/종료destroy·builder삭제는미검증또는명시경계다([실행근거](PRODUCT-TRUTH.md), [감사](../docs/audits/phase-10-15-repair.md)). cleanup Q024는별도이며새실행승인대기가아님 |
...
| Q-024 | Phase11 종료 inventory의 dashboard API 경로를 보완해야 하는가? | 2026-08-26 | static-gap — advanced.py의삭제후inventory는dashboard list에도v3 base를사용한다. 실제apply/verify는별도monitoring.py의v1으로통과했다. 다음명시적종료요청에서v1 list·읽기inventory를수정검증후새삭제계획필요. 현재실기소스binding을유지하며전체destroy성공을주장하지않음 |
