...
| Q-014 | D-036의 보존·진단·수정·재apply 원칙을 실제 자동화의 모든 실행/실패 경로에 어떻게 이관할까? | 2026-08-26 | partial — Phase09 전용 recovery.sh/py의 보존·동일 state replan은 오프라인 검사와 D-039 실제 apply10개·verify 실패 후 VM2/SQL1/state 보존으로 관측했다. Phase08 승인 소스 보존을 위해 shared adapter/config와 다른 Phase는 미변경이며 기존 auto-destroy 경로 실행 금지 유지 |
