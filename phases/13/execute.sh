...
phase_write_tfvars() {
  jq -n --arg project_id "$GCP_PROJECT_ID" --arg run_id "$2" --arg region "$GCP_REGION" --arg zone "$GCP_ZONE" --arg secondary_region "$GCP_SECONDARY_REGION" \
    '{project_id:$project_id,run_id:$run_id,region:$region,zone:$zone,secondary_region:$secondary_region}' >"$1"
}
...
phase_after_apply() {
  local run_id="$1" run_dir serial apache_version base_image provenance builder_status
