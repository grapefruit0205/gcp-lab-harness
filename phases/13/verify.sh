...
if [[ "$mode" == destroyed ]]; then
  ...
  python3 "$repo_root/lib/harness/advanced.py" inventory --phase 13 --run-dir "$repo_root/artifacts/runs/$run_id/phase-13"
  exit 0
fi
[[ "${HARNESS_SAFE_VERIFY_RUN:-}" == "$run_id" ]] || harness_die "승인 source/account 검사를 위해 phases/13/execute.sh verify --run $run_id 로 실행하세요."
...
jq -e '.builder_ready==true and .builder_stopped_before_image==true and .reset_autostart_verified==true and .distinct_ready_boots>=2 and (.base_image_sha256|test("^[a-f0-9]{64}$")) and (.apache_package_version|length>0)' "$provenance" >/dev/null || harness_die "custom image provenance evidence 불일치"
...
[[ "$(gcloud compute instances describe "p13-builder-$run_id" --zone="$GCP_ZONE" --project="$GCP_PROJECT_ID" --format='value(status)')" == TERMINATED ]] || harness_die "builder가 안전한 중지 상태가 아닙니다."
...
jq -e --arg primary "us-1-mig-$run_id" --arg secondary "notus-1-mig-$run_id" '
  (.backends|length)==2 and
  any(.backends[]; (.group|endswith("/"+$primary)) and .balancingMode=="RATE" and .maxRatePerInstance==50) and
  any(.backends[]; (.group|endswith("/"+$secondary)) and .balancingMode=="UTILIZATION" and .maxUtilization==0.8)
' <<<"$backend_json" >/dev/null || harness_die "원문 RATE50/UTILIZATION80 backend 계약 불일치"
backend_healthy() {
  local health
  health="$(gcloud compute backend-services get-health "$backend" --global --project="$GCP_PROJECT_ID" --format=json)" || return 1
  printf %s "$health" >"$evidence_dir/backend-health.json"
  python3 "$repo_root/lib/harness/advanced.py" health --file "$evidence_dir/backend-health.json" --expected "us-1-mig-$run_id" "notus-1-mig-$run_id"
}
...
for version in ipv4 ipv6; do
  forward="$(gcloud compute forwarding-rules describe "p13-http-$version-$run_id" --global --project="$GCP_PROJECT_ID" --format=json)"
  expected_address="$ipv4"; [[ "$version" != ipv6 ]] || expected_address="$ipv6"
  jq -e --arg ip "$expected_address" --arg proxy "p13-http-proxy-$run_id" '.IPAddress==$ip and .IPProtocol=="TCP" and .loadBalancingScheme=="EXTERNAL_MANAGED" and (.target|endswith("/"+$proxy)) and (.portRange=="80-80" or .portRange=="80")' <<<"$forward" >/dev/null || harness_die "ALB $version frontend readback 불일치"
done
ipv6_probe=unavailable
if ip -6 route get "$ipv6" >/dev/null 2>&1; then
  harness_wait_until 300 10 curl -6 -fsS --max-time 10 "http://[$ipv6]/" >/dev/null || harness_die "IPv6 route는 있으나 frontend HTTP 실패"
  ipv6_probe=passed
fi
...
jq -n --arg phase 13 --arg run_id "$run_id" --arg ipv6_probe "$ipv6_probe" --arg provenance_sha256 "$(harness_sha256_file "$provenance")" --argjson initial_total "$initial_total" --argjson scale_out_total "$scale_out_total" --argjson distinct_markers "$distinct_markers" --argjson lb_log_count "$lb_log_count" '{phase:$phase,run_id:$run_id,tasks:{
 ...
 "task-2":{status:"passed",detail:"private builder와 세 번째 region loadgen의 Cloud NAT 확인"},
  "task-3":{status:"passed",detail:"reset 전후 Apache 자동기동과 READY image/중지 builder 확인; builder 삭제는 보존 경계"},
