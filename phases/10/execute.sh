...
phase_preflight(){ command -v bq >/dev/null; export P10_FIXTURE_URI="${P10_FIXTURE_URI:-gs://cloud-training/archinfra/BillingExport-2020-09-18.avro}"; [[ "$P10_FIXTURE_URI" == gs://* ]]||{ printf 'FAIL: GCS fixture URI 필요\n' >&2;return 1;}; }
...
phase_write_action_plan(){
  ...
  [[ -n "$generation" && -n "$crc" ]]||{ printf 'FAIL: fixture generation/checksum 조회 실패\n' >&2;return 1;}
  jq -n --arg run_id "$2" --arg uri "$P10_FIXTURE_URI" --arg generation "$generation" --arg crc "$crc" '{schema_version:1,phase:"10",run_id:$run_id,actions:[
    {id:"fixture-load",kind:"bq",target:($uri+"#"+$generation+" crc32c="+$crc),mutation:"AVRO load to run dataset",rollback:"dataset destroy",timeout_seconds:900,contains_secret:false},
