...
if [[ "$mode" == destroyed ]]; then
  ...
  python3 "$repo_root/lib/harness/advanced.py" inventory --phase 15 --run-dir "$repo_root/artifacts/runs/$run_id/phase-15"
  exit 0
fi
[[ "${HARNESS_SAFE_VERIFY_RUN:-}" == "$run_id" ]] || harness_die "승인 source/account 검사를 위해 phases/15/execute.sh verify --run $run_id 로 실행하세요."
...
state_json="$(terraform -chdir="$work_dir" show -json)"
addresses="$(jq -r '[.. | objects | select(.mode? == "managed" and has("address") and has("values")) | .address] | unique | .[]'<<<"$state_json")"
[[ "$(wc -l<<<"$addresses")" -eq 4 ]]||harness_die "Terraform managed state address가 4개가 아닙니다(data source 제외)."
...
harness_wait_until 300 10 timeout 60 gcloud compute ssh "$vm1" --zone="$GCP_ZONE" --project="$GCP_PROJECT_ID" --tunnel-through-iap --quiet --command="ping -c 3 -W 3 '$vm2_ip' >/dev/null"||harness_die "두 module VM private ping 실패"

idempotency_plan="$run_dir/phase-15-idempotency.tfplan";set +e;harness_tf_timeout terraform -chdir="$work_dir" plan -input=false -lock-timeout=30s -detailed-exitcode -out="$idempotency_plan" >"$run_dir/idempotency.log" 2>&1;plan_rc=$?;set -e
