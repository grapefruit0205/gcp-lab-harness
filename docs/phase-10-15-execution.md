...
이 문서는 저장소를 clone한 사용자가 **자신의 Google 계정·프로젝트**로 실행하는 절차다. 2026-08-26에 Phase10–15 모두 실제 apply와 기계 검증을 통과했다(각 run 1회 성공 관측). 로컬54개 회귀·Terraform mock9개·전체 offline 검사도 통과했다. 사용자 콘솔 클릭·수동 항목·최종 destroy까지 완료했다는 뜻은 아니다. [수정 내역과 남은 한계](audits/phase-10-15-repair.md)를 먼저 읽는다.

이번 유지관리 작업 예외(D-047, 2026-08-26): 사용자가 Phase10–15의 구현·apply·오류 수정을 재질문 없이 명시 위임했다. 담당자가 각 저장 계획·정확한 SHA·소유권·비용을 확인·기록한 뒤 실행했고 개별 SHA 승인을 다시 요청하지 않았다. 이 실행은 완료됐으며 **새 실행이나 clone 사용자에게 위임이 자동 승계되지 않는다.** 아래의 일반 승인 절차와 기술적 무결성 검사는 그대로다. 다른 계정/프로젝트·이전 실습 변경·전체 destroy·외부 계정 권한 확대는 위임 대상이 아니었다.

### 이번 실측 결과와 콘솔 안내

| Phase / run | 실제 확인 | Task 하위 항목별 확인법 |
|---|---|---|
| 10 / `p10-260826-2106` | 415602행·시간 TIMESTAMP·쿼리8개 | [BigQuery 작업·결과](console/phase-10.md) |
| 11 / `p11-260826-2224` | VM3 metrics/group·uptime 최신 true·alert Off | [Monitoring 그룹·차트·정책](console/phase-11.md) |
| 12 / `p12-260826-2236` | BGP·라우팅 전이·단일 터널 장애 후 통신 | [VPN·Router·장애 상태](console/phase-12.md) |
| 13 / `p13-260826-2243` | HTTP·backend marker4·LB로그20·자동 확장/축소 | [이미지·MIG·외부 LB](console/phase-13.md) |
| 14 / `p14-260826-2300` | backend2 healthy·VIP60응답·client IP 보존 | [내부 LB·VM·VIP](console/phase-14.md) |
| 15 / `p15-260826-2300` | private ping·managed4개·재plan 변경0 | [VPC·VM·멱등성 증거](console/phase-15.md) |

Phase12는 의도적 장애 실습 후 터널 하나가 삭제된 상태이며 Task8 정리는 미수행이다. Phase13은 IPv6 구성만 확인했고 로컬 IPv6 경로 부재로 실제 IPv6 HTTP는 미검증이다. 원문 builder 삭제 대신 중지 보존했다. 각 결과의 정확한 SHA·증거·한계는 [실행 근거](../memory/PRODUCT-TRUTH.md)에 있으며 리소스 유지 비용은 계속 발생한다.
...
현재 알려진 종료 전 보완: Phase11의 삭제 후 dashboard 목록 조회에는 API 버전 오류가 남아 있다(Q-024). 다음 명시적 종료 요청에서 해당 inventory의 v1 경로를 먼저 수정·검증한 뒤 새 삭제 계획을 만든다. 아래 명령 예시는 Phase10이며 Phase11 cleanup의 실기 성공을 뜻하지 않는다.
