...
이 검사는 15개 문서와 원본 Task 번호·하위 제목의 누락·중복·빈 기준·세부 절차·보고 연결을 검사한다. Cloud 성공이나 실제 콘솔 클릭 전 과정 성공의 증거는 아니다.

## Task 아래 상세 항목만 보기

각 Phase 요약표 바로 아래의 **하위 항목별 상세 확인** 링크를 엽니다. 원문 하위 제목마다 클릭 경로, 읽을 값, 판정과 한계를 나눴습니다. 원문에 하위 제목이 없는 SQL·BigQuery 작업은 번호 절차를 별도 점검 항목으로 분할했습니다.

예를 들어 Phase10의 Task4 분석 쿼리7개만 읽으려면 다음 로컬 명령을 사용합니다.

```bash
python3 scripts/console-checks.py --phase 10 --task 4
```

원문 하위 제목은 **새로 만들라는 지시가 아니라 이미 실행한 결과를 확인할 항목명**입니다. 가이드의 첫 번째 줄에서 열 화면과 대상, 두 번째·세 번째 줄에서 통과 값과 수동 경계를 읽으세요. 콘솔로 확인할 수 없는 과거 전이는 정제 evidence로 구분합니다.
