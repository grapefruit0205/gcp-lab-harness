...
if rg -F 'source_ranges = ["0.0.0.0/0"]' "$repo_root/phases"/{01,02,03,04,05,06}/terraform >/dev/null; then
  printf 'FAIL: Phase 01–06에 public 전체 ingress가 있습니다.\n' >&2
  exit 1
fi

if rg -F 'scopes = ["cloud-platform"]' "$repo_root/phases"/{01,02,03,04,05,06}/terraform >/dev/null; then
